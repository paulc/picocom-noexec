		...
		if ( FD_ISSET(STI, &rdset) ) {
			...
			case ST_COMMAND:
				...
				case KEY_LECHO:
					...
					break;
				case KEY_SEND:
                    if (strlen(opts.send_cmd) > 0) {
                        fd_printf(STO, "\r\n*** file: ");
                        r = fd_readline(STI, STO, fname, sizeof(fname) - 2);
                        fd_printf(STO, "\r\n");
                        if ( r < -1 && errno == EINTR ) break;
                        if ( r <= -1 )
                            fatal("cannot read filename: %s", strerror(errno));
                        /* Samitise and quote fname */
                        for (char *c = fname; *c != 0; ++c) 
                            if (*c == '\'') *c = '_';
                        sprintf(fname_quote,"'%s'",fname);
                        run_cmd(tty_fd, opts.send_cmd, fname_quote, NULL);
                    } else {
                        fd_printf(STO, "\r\n*** send command not enabled ***\r\n");
                    }
					break;
				case KEY_RECEIVE:
                    if (strlen(opts.receive_cmd) > 0) {
                        fd_printf(STO, "*** file: ");
                        r = fd_readline(STI, STO, fname, sizeof(fname) - 2);
                        fd_printf(STO, "\r\n");
                        if ( r < -1 && errno == EINTR ) break;
                        ...
                        if ( fname[0] ) {
                            ...
                            for (char *c = fname; *c != 0; ++c) 
                                if (*c == '\'') *c = '_';
                            sprintf(fname_quote,"'%s'",fname);
                            run_cmd(tty_fd, opts.receive_cmd, fname_quote, NULL);
                        } else
                            run_cmd(tty_fd, opts.receive_cmd, NULL);
                    } else {
                        fd_printf(STO, "\r\n*** receive command not enabled ***\r\n");
                    }
					break;
				case KEY_BREAK:
					term_break(tty_fd);
...
parse_args(int argc, char *argv[])
{
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "nqhirlcv:s:r:e:f:b:p:d:",
	...
	printf("nolock is      : %s\n", opts.nolock ? "yes" : "no");
#endif
        printf("send_cmd is    : %s\n", strlen(opts.send_cmd) ? opts.send_cmd : "<disabled>");
        printf("receive_cmd is : %s\n", strlen(opts.receive_cmd) ? opts.receive_cmd : "<disabled>");
